...
install -m 0755 "$stage-relay" /usr/local/bin/wt-agent-git-relay
install -m 0755 "$stage-remote" /usr/local/bin/git-remote-ag
install -m 0755 "$stage-cli" /usr/local/bin/ag-git
install -d -m 0700 -o "$WT_USER" -g "$WT_USER" /var/lib/wt-agent-git
install -m 0600 -o "$WT_USER" -g "$WT_USER" "$stage-grant" /var/lib/wt-agent-git/grant
install -m 0600 -o "$WT_USER" -g "$WT_USER" \
    "$stage-providers" /var/lib/wt-agent-git/providers
...
if ! systemctl enable --now wt-agent-git-relay.service; then
    systemctl status --no-pager --full wt-agent-git-relay.service >&2 || true
    journalctl --no-pager -u wt-agent-git-relay.service -n 100 >&2 || true
    exit 1
fi
