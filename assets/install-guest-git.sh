...
if test "$#" -ne 6; then
    echo 'usage: install-guest-git.sh SOURCE CLONE CHECKOUT VALUE USER_NAME USER_EMAIL' >&2
    exit 2
fi
...
cleanup() {
    rm -rf "$runtime" "$askpass" \
        "$stage-identity" "$stage-known-hosts" "$stage-passphrase"
}
...
rm -rf /workspace/.git/wt
/usr/bin/git -c safe.directory=/workspace -C /workspace config --local --unset-all core.sshCommand ||
    test "$?" -eq 5
