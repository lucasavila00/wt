...
Virtiofs passthrough preserves numeric ownership. Guest Codex runs as fixed
`wt:wt` UID/GID `1001:1001` and creates `0600` rollouts, but host bootstrap
currently allocates `wt` dynamically. Equal names do not grant access when the
numbers differ. WT hosts may be rebuilt from scratch, so migration compatibility
is not required.
