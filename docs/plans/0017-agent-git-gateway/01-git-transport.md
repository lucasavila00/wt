...
3. Add the provider configuration from the main plan. Require at least one
...
5. Configure `origin` with an `ag::` URL. Stream the real Git protocol through
   ...
   and `git push` work.
6. Enforce the world's branch prefix at the gateway. Allow updates,
   force-pushes, and deletion inside that prefix. Reject tags and every other
   branch with the message from the ADR.
7. Install the checkout and commit hints without replacing project hooks.
   Snapshot the complete `ag-git --help` output and all user-visible messages.
   Provider-backed commands fail clearly until milestone 2 or 3 implements the
   selected provider.
8. Update `DEVELOPMENT.md` and
