...
- Named local and OpenSSH contexts carry the same versioned `wt-local` JSON API.
...
- Sites are trusted credential boundaries, not hostile multi-tenant sandboxes.
- There is no public control-plane listener and no runtime configuration override;
  each site uses the complete strict config at `/etc/wt/local.toml`.
...
- Git checkout and credentials remain inside the trusted world. Client-to-site
