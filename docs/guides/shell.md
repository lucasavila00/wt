...
refreshes cards only for Byobu panes whose foreground process is `codex`, with a
preview rendered from the persistent SSH/PTY playback connection already owned by
each world. `Enter` or left click on a Live preview verifies and selects its
matching Codex pane in Byobu, then opens that world's full Byobu view. The focus
