...
| `wt-libvirt` | KVM machine lifecycle implementation |
| `wt-static-ssh` | Existing-machine lifecycle and pinned OpenSSH transport |

The server selects one backend. Start with one backend per server rather than
selecting a provider for each world.
...
The client context selects the server and infrastructure pool.
