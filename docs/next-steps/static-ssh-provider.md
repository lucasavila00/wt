...
The server selects the `static_ssh` backend and keeps the machine destination
and bootstrap identity in server configuration. Unlike the libvirt backend,
this backend claims an existing machine instead of creating a KVM guest.

## Target requirements

The target is a dedicated Ubuntu 24.04 amd64 VM with root or passwordless sudo
access. It does not require KVM.
...
Deletion removes only WT-owned resources. It must never stop or delete the VM.
