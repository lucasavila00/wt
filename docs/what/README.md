...
`NAME`, `NAME-vs`, and `NAME-vc` require the injected app SSH server. `NAME`
...
| `NAME-vs`, `NAME-vc` | App SSH server through the guest proxy | Workstation's world key | Direct app SSH |
