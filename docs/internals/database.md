...
the latest fingerprint, change/freshness timestamps, current working directory,
and (when that directory has a `.git` folder) checked-out branch for each
observed Byobu pane. Both are deleted with their world. It never stores rendered
terminal contents or Codex lifecycle events.
