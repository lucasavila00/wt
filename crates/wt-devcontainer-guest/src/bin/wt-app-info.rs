    ...
    let result = match args.as_slice() {
        [command] if command == "configured-user" => {
            ...
            std::io::stdin()
                ...
                .and_then(|_| wt_devcontainer_guest::configured_remote_user(&input))
                .map(Some)
        }
        [command, expected] if command == "verify-user" => {
            wt_devcontainer_guest::verify_app_user(expected).map(|_| None)
        }
        [] | [_] => wt_devcontainer_guest::app_target()
