...
#[derive(Debug, Parser)]
#[command(name = "wt-agent-git-gateway-relay")]
struct Cli {
    #[arg(long, default_value = "/run/wt-agent-git-gateway/gateway.sock")]
    socket: PathBuf,
    #[arg(long, default_value = "/var/lib/wt-agent-git-gateway/grant")]
    grant_file: PathBuf,
    #[arg(long)]
...
fn main() {
    if let Err(error) = run() {
        eprintln!("wt-agent-git-gateway-relay: {error:#}");
        std::process::exit(1);
    }
}
            ...
            Ok(stream) => {
                ...
                std::thread::spawn(move || {
                    if let Err(error) = handle(stream, &token, gateway_unix, vsock_port) {
                        eprintln!("wt-agent-git-gateway-relay: request: {error:#}");
                    }
                });
            }
            Err(error) => eprintln!("wt-agent-git-gateway-relay: accept: {error}"),
        }
    }
    Ok(())
}
...
fn handle(
    ...
    let streams_git = matches!(
        &request.operation,
        wt_agent_git_gateway::ClientOperation::Git { .. }
    );
