...
const AGENT_TOOLS_HINT: &[u8] =
    include_bytes!("../../../../../../assets/world/devcontainer/agent-tools-hint.sh");
...
impl WorldProvisioner {
    ...
    pub fn provision(
        ...
        wt_control_protocol::validate_ssh_git_source(spec.source)
        ...
        let host_keys = crate::read_host_keys(transport, deadline)?;
    ...
    fn inspect_with_deadline(
        ...
        let app_ssh = if complete {
            ...
            Some(wt_control_protocol::AppSshAccess {
                user: target.user,
                port: APP_SSH_PORT,
                host_keys,
            })
    ...
    fn bootstrap(
        ...
        for (suffix, contents) in [
            ...
            ("-agent-tools-hint", AGENT_TOOLS_HINT),
        ...
        let _ = guest::exec(
            ...
            &[
                ...
                "/tmp/wt-install-guest-agent-tools-hint",
    ...
    fn configure_and_verify_app_ssh(
        ...
        if !crate::host_keys_match(&expected, &String::from_utf8_lossy(&scanned)) {
            return Err(WorkerError::new(
                "app SSH readiness: presented host keys do not match the per-world identity",
            ));
        }
        let known_hosts = crate::normalized_host_keys(&expected.join("\n"))
            .into_iter()
