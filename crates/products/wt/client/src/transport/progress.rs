...
mod tests {
    ...
    fn frame_kind_is_explicit_and_malformed_events_are_not_responses() {
        assert!(matches!(
            decode_frame(
                &context(),
                br#"{"protocol_version":10,"event":"progress","message":"waiting"}"#,
            )
            .unwrap(),
        ...
        assert!(decode_frame(
            &context(),
            br#"{"protocol_version":10,"event":"future","message":"waiting"}"#,
        )
        .is_err());
    }
    ...
    fn frame_rejects_a_wrong_protocol_version() {
        ...
        assert!(error.body().contains("expected 10"));
    }

    #[test]
    fn terminal_response_is_unique_and_last() {
        let response_line = br#"{"protocol_version":10,"outcome":"ok","response":{"response":"instances","instances":[],"disk_usage_bytes":{},"agent_tool_report_counts":{}}}"#;
        let mut response = None;
        assert!(accept_frame(
            &mut response,
        ...
        assert!(accept_frame(
            ...
            decode_frame(
                &context(),
                br#"{"protocol_version":10,"event":"progress","message":"late"}"#,
            )
            .unwrap()
        )
        .is_err());
