...
fn reconcile_home(codex: &Path, home: &Path) -> Result<()> {
    reconcile_home_with_timeout(codex, home, RESPONSE_TIMEOUT)
}

fn reconcile_home_with_timeout(
    codex: &Path,
    home: &Path,
    response_timeout: Duration,
) -> Result<()> {
    let mut server = AppServer::start(codex, home, response_timeout)?;
...
impl AppServer {
    fn start(codex: &Path, home: &Path, response_timeout: Duration) -> Result<Self> {
    ...
    fn call<T: DeserializeOwned>(&mut self, method: &str, params: Value) -> Result<T> {
        ...
        loop {
            match self.output.recv_timeout(self.response_timeout) {
...
"#,
    ...
    fn fails_when_codex_app_server_stops_replying() {
        ...
        insta::assert_snapshot!(
            reconcile_home_with_timeout(&codex, &home, Duration::from_millis(10))
                .unwrap_err()
                .to_string(),
