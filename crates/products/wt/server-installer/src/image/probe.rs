...
pub(super) fn verify_publication(
    ...
    let guest_uid = wt_retained_worlds::GUEST_UID.to_string();
    let guest_gid = wt_retained_worlds::GUEST_GID.to_string();
    ...
    let result = (|| {
        ...
        run_guest(
            ...
            wt_retained_worlds::MOUNT_CODEX_HELPER,
            &[],
            deadline,
...
fn validate_marker_details(
    ...
    if kind == "file"
        && uid == wt_retained_worlds::GUEST_UID
        && gid == wt_retained_worlds::GUEST_GID
        && mode == 0o600
