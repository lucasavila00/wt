...
const CODEX_CONFIG: &[u8] =
    include_bytes!("../../../../../../assets/world/guest/codex-config.toml");
...
pub(super) fn run_kvm_build<R: Runner>(
    ...
    let codex_config = build_dir.join("codex-config.toml");
    ...
    fs::write(&codex_config, CODEX_CONFIG).context("write Codex configuration")?;
    ...
    for (source, guest_path) in [
        ...
        (codex_config.as_path(), "/var/tmp/wt-codex-config.toml"),
