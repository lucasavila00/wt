    ...
    fn execute_command(
        ...
        match command {
            ...
            ProviderCommand::CancelCiJob { job } => {
                self.require_ci_job(scope, job)?;
                bail!(
                    "GitHub cannot cancel one Actions job: its API can only cancel the entire workflow run, including sibling jobs; wt-tools refuses to widen `cancel {job}` beyond the selected job"
                )
            }
