...
fn main() {
    if let Err(error) = run() {
        eprintln!("wt-agent-tool-gateway: {error:#}");
        std::process::exit(1);
    }
}
                ...
                Ok(stream) => {
                    ...
                    std::thread::spawn(move || {
                        if let Err(error) = gateway.handle_control(stream) {
                            eprintln!("wt-agent-tool-gateway: control request: {error:#}");
                        }
                    });
                }
                Err(error) => eprintln!("wt-agent-tool-gateway: accept control request: {error}"),
            }
        }
    });
        ...
        std::thread::spawn(move || {
            for stream in listener.incoming() {
                match stream {
                    ...
                    Err(error) => {
                        eprintln!("wt-agent-tool-gateway: accept transport request: {error}")
                    }
                }
            }
        });
...
fn fixture_api() -> Result<Option<FixtureApi>> {
    let kind = std::env::var("WT_AGENT_TOOL_TEST_PROVIDER_KIND").ok();
    let base_url = std::env::var("WT_AGENT_TOOL_TEST_API_BASE").ok();
    let token_file = std::env::var_os("WT_AGENT_TOOL_TEST_TOKEN_FILE").map(PathBuf::from);
    match (kind, base_url, token_file) {
        ...
        (Some(kind), Some(base_url), Some(token_file)) => {
            let kind = match kind.as_str() {
                ...
                _ => anyhow::bail!("WT_AGENT_TOOL_TEST_PROVIDER_KIND must be github or gitlab"),
            };
            Ok(Some(FixtureApi {
                kind,
                base_url,
        ...
        _ => anyhow::bail!(
            "WT agent tool test API requires provider kind, base URL, and token file together"
        ),
    }
}
