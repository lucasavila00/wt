...
mod tests {
    ...
    fn world_prompt_explains_the_disposable_world_and_wtg_tools() {
        ...
        This environment has wtg tools installed for pull or merge request, review, and CI operations; run wtg tools help to see its supported commands. Use normal Git for commits, fetches, pulls, and pushes. The Git gateway can read every available repository and requires branch names to use the shared `wt/` prefix (for example, `wt/fix-login`). Every WT world may create branches and push fast-forward updates under `wt/`, so an agent can continue or take over work from another agent. The gateway rejects history rewrites and branch deletions. If the gateway rejects a branch, rename it with git branch -m wt/NAME.

        PRs are usually squash-merged. Preserve the PR's development and review history with follow-up commits; do not rebase, amend published commits, or force-push as routine cleanup. Merge the base branch into the PR branch when needed. If a history rewrite is needed, prepare and push the replacement commits to a new `wt/` branch, then ask the user to update the original PR branch from their laptop using their own credentials. Provide the repository, both branch names, the original branch's expected remote SHA, the replacement SHA, and exact fetch and push commands using `--force-with-lease=<ref>:<expected-old-sha>` and the replacement SHA as the push source. If the original branch moves, reassess instead of refreshing the lease blindly. The gateway cannot perform this rewrite.
        "###);
    }
