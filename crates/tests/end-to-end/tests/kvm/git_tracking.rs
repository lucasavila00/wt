...
fn branch_changes_during_a_working_turn_without_another_codex_hook() {
    ...
    run_guest(
        ...
        &name,
        &format!(
            "set -eu; git clone https://local.test/acme/widget.git /home/wt/project; cd /home/wt/project; git switch -c wt/tracker-before; tmux new-session -d -s wt-host -n git-tracker 'cd /home/wt/project; exec bash'; pane=$(tmux list-panes -t wt-host:git-tracker -F '#{{pane_id}}'); tmux send-keys -t \"$pane\" \"printf '%s\\n' '{{\\\"session_id\\\":\\\"{session_id}\\\",\\\"cwd\\\":\\\"/home/wt/project\\\",\\\"hook_event_name\\\":\\\"UserPromptSubmit\\\"}}' | WT_BYOBU_SESSION=wt-host WT_BYOBU_PANE=$pane wtg codex report-hook\" C-m; for attempt in $(seq 1 50); do test -f /home/wt/.local/state/wt/codex-git-tracker.json && break; sleep 0.1; done; test -f /home/wt/.local/state/wt/codex-git-tracker.json; git switch -c wt/tracker-after",
        ),
