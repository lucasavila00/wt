...
fn host_create_returns_setup_then_reconciles_running() {
    ...
    assert_eq!(instance.kind(), wt_control_protocol::WorldKind::Host);
...
fn create_rejects_a_world_larger_than_host_memory() {
    ...
    .unwrap_err();
    assert_eq!(error.code, wt_control_protocol::ErrorCode::InvalidRequest);
...
fn concurrent_creates_cannot_claim_the_same_memory() {
    ...
    assert_eq!(
        results
            .iter()
            .filter(
                |result| matches!(result, Err(error) if error.code == wt_control_protocol::ErrorCode::Capacity)
            )
            .count(),
        1
    );
...
fn matching_retry_waits_for_synchronous_preparation() {
    ...
    .unwrap_err();
    assert_eq!(delete_error.code, wt_control_protocol::ErrorCode::Conflict);
...
fn set_git_base(request: &mut wt_control_protocol::CreateInstance, value: &str) {
