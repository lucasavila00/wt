...
fn run_trampoline(args: Vec<OsString>) -> Result<()> {
    ...
    match reconcile::reconcile_with_codex(&real_codex) {
        ...
        Err(error) => eprintln!("wt-codex: reconciliation failed: {error:#}"),
    }
...
"###
    ...
    fn remove_is_an_alias_for_uninstall() {
        assert!(matches!(
            Cli::try_parse_from(["wt-codex", "remove"]).unwrap().command,
            Command::Uninstall
        ));
    }
}
