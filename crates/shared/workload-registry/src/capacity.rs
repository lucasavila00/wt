...
use crate::{to_i64, to_u64, GuestRow, RegistryError, Resource, Resources};
use diesel::prelude::*;
...
pub fn reserved_resources(connection: &mut SqliteConnection) -> Result<Resources, RegistryError> {
    let sum = diesel::sql_query(
        "SELECT COALESCE(SUM(CASE WHEN compute_reserved THEN vcpus ELSE 0 END), 0) AS vcpus, COALESCE(SUM(CASE WHEN compute_reserved THEN memory_mib ELSE 0 END), 0) AS memory_mib, COALESCE(SUM(disk_reserved_gib), 0) AS disk_gib FROM guests",
    )
...
pub fn reserve_resources(
    ...
    let row = guests::table
        .find(id.to_string())
        .select(GuestRow::as_select())
        .first::<GuestRow>(connection)?;
    ...
    diesel::update(guests::table.find(id.to_string()))
        .set((
            guests::compute_reserved.eq(true),
            guests::disk_reserved_gib.eq(to_i64(disk_target_gib, "disk_reserved_gib")?),
        ))
        .execute(connection)?;
    Ok(())
}
...
pub fn release_resources(
    ...
    let changed = diesel::update(guests::table.find(id.to_string()))
        .set((
            guests::compute_reserved.eq(false),
            guests::disk_reserved_gib.eq(to_i64(disk_reserved_gib, "disk_reserved_gib")?),
        ))
        .execute(connection)?;
    if changed == 1 {
        Ok(())
...
pub fn ensure_resources_reserved(
    ...
    let changed = diesel::update(guests::table.find(id.to_string()))
        .set((
            guests::compute_reserved.eq(true),
            guests::disk_reserved_gib.eq(to_i64(
